function [gnssRaw,gnssAnalysis] = ReadLogFile(dirName,CustomFileName,GnssFileName,dataFilter)
%% [gnssRaw,gnssAnalysis]=ReadLogFile(dirName,CustomFileName,GnssFileName,dataFilter);
% Read the log files created by Gnss Logger App in Android
% Compatible with Android release N
%
% Input: 
%  dirName = string with directory of fileName, 
%                e.g. '~/Documents/MATLAB/Pseudoranges/2016-03-28'
%  CustomFileName = string with filename in the format our own
%  GnssFileName = string with filename created by the app
%  optional inputs:
...
if nargin<4, dataFilter = []; end

%% check we have the right kind of fileName
extension = GnssFileName(end-3:end);
if ~any(strcmp(extension,{'.txt','.csv'}))
    error('Expecting file name of the form "*.txt", or "*.csv"');
end

extension = CustomFileName(end-3:end);
if ~any(strcmp(extension,{'.txt','.csv'}))
    error('Expecting file name of the form "*.txt", or "*.csv"');
end

%% read log file into a numeric matrix 'S', and a cell array 'header'
rawCsvFile = MakeCsv(dirName,GnssFileName);
[header,C] = ReadRawCsv(rawCsvFile);

%% add the information of ours
header = [header;'AngleZ';'AngleX';'AngleY'];
AngleZ = [];
AngleX = [];
AngleY = [];
fid = fopen([dirName,CustomFileName]);
    while ~feof(fid)
        line = fgetl(fid);
        ...
        args = sscanf(line, "%u,%u,%u,%f,%f,%f,%f", [7,1]);
        AngleZ = [AngleZ;args(5)];
        AngleX = [AngleZ;args(6)];
        AngleY = [AngleZ;args(7)];
    end
C = [C,AngleZ,AngleX,AngleY];
